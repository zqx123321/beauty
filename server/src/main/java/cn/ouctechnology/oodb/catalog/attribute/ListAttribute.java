...
public class ListAttribute extends Attribute {
    ...
    public void writeValue(Block block, Object value, int offset) {
        if (value instanceof List) {
            ...
            writeValueList(block, valueList, offset);
        } else {
            Tuple tuple = (Tuple) value;
            writeValueTuple(block, tuple, offset);
        }
    }
     ...
     * 只有value的顺序式写入
     */
    private void writeValueList(Block block, List<Object> valueList, int offset) {
        //没赋值的为默认值
...
//            throw new IllegalArgumentException("the value count does not match the list count,excepted is:" + size + ",now is:" + valueList.size());
        for (Object value : valueList) {
            listAttribute.writeValue(block, value, offset);
            offset += listAttribute.getLength();
        }
    }
     ...
     * 有key和value的跳跃式写入
     */
    private void writeValueTuple(Block block, Tuple tuple, int offset) {
        //补偿递归产生的额外的offset
        //int attributeOffset = Catalog.getAttributeOffset(block.getBlockKey().filename, this.name);
        block.setDataOffset(block.getDataOffset() - offset);
    ...
    public Attribute getAttribute(String attributeName) {
        ...
        try {
            if (!leftField.contains(".")) {
                ...
                String indexField = OgnlUtil.getField(leftField, 0);
                int index = Integer.parseInt(indexField);
                if (index < 0 || index >= size) throw new IndexOutOfBoundsException(index + "");
                return listAttribute.getAttribute(OgnlUtil.getLeftField(leftField));
            }
        } catch (Exception e) {
            return null;
        }
